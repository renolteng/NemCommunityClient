...
public class KeyPairsStorage {
	...
	public static void save(final List<AliasedKeyPair> keyPairs, final CommandLine commandLine) {
		save(keyPairs, commandLine.getOptionValue("output"), commandLine.getOptionValue("pass"));
	}

	public static void save(final List<AliasedKeyPair> keyPairs, final String outputFileName, final String password) {
		final byte[] buffer = BinarySerializer.serializeToBytes(serializer -> serializer.writeObjectArray("tuples", keyPairs));
		final EncryptedInputOutput io = new EncryptedInputOutput(password);
	...
	public static List<AliasedKeyPair> load(final CommandLine commandLine) {
		return load(commandLine.getOptionValue("input"), commandLine.getOptionValue("pass"));
	}

	public static List<AliasedKeyPair> load(final String inputFileName, final String password) {
		final EncryptedInputOutput io = new EncryptedInputOutput(password);
